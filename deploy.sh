...
# Step 3: Check if Azure Container Registry (ACR) exists
...
# Step 5: Check if the "podmanagerlive" repository exists in ACR
echo "Checking if repository 'podmanagerlive' exists in ACR..."
if az acr repository list --name $REGISTRY_NAME --output tsv | grep -q "podmanagerlive"; then
...
# Step 9: Check if App Service Plan exists
...
# Step 10: Check if Web App exists
...
# Step 11: Configure Web App to use ACR with Managed Identity (No publish profile required)
...
# Step 13: Optionally, open the Web App in a browser
