...
# Step 4: Check if the "podmanager" repository exists in ACR
echo "Checking if repository 'podmanager' exists in ACR..."
if az acr repository show --name $REGISTRY_NAME --repository podmanager --output none; then
    echo "Repository 'podmanager' exists. Deleting repository and its contents..."
    az acr repository delete --name $REGISTRY_NAME --repository podmanager --yes --output none
else
    echo "Repository 'podmanager' does not exist. No need to delete."
fi
...
# Step 10: Configure Web App to use ACR
...
# Step 11: Assign ACR Pull Role to Web App
echo "Assigning ACR pull role to Web App..."
...
az role assignment create --assignee $WEBAPP_ID --role "AcrPull" --scope $ACR_ID
