...
def register_podtask():
    ...
    try:
        ...
        result = collection.database["Podtask"].insert_one(
            podtask_item
        )  # Ensure "Podtask" is correct
    ...
    try:
        ...
        update_fields = {
            ...
            "UrlDescribe": data.get(
                "externalurl", existing_task["UrlDescribe"]
            ).strip(),
            "SubimissionReq": (
...
@podtask_bp.route("/get_default_tasks", methods=["GET"])
def get_default_tasks():
    ...
    try:
        ...
        return jsonify({"default_tasks": default_tasks}), 200

    except Exception as e:
        print(f"❌ ERROR: {e}")
        return jsonify({"error": f"Failed to fetch default tasks: {str(e)}"}), 500


@podtask_bp.route("/delete_default_task/<task_id>", methods=["DELETE"])
def delete_default_task(task_id):
    if not g.user_id:
        return jsonify({"error": "Unauthorized"}), 401

    try:
        result = collection.database.DefaultTasks.delete_one({"_id": task_id})

        if result.deleted_count == 1:
            return jsonify({"message": "Default task deleted successfully"}), 200
        else:
            return jsonify({"error": "Failed to delete default task"}), 500
