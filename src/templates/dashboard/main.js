...
function populateLeaderboard(leaderboard) {
  ...
  function connectGoogleCalendar() {
    ...
    const userApiKey = prompt(
      "Please enter your Google API Key to connect your calendar:"
    );
    ...
    if (userApiKey) {
      ...
      fetch(
        `https://www.googleapis.com/calendar/v3/calendars/primary/events?key=${userApiKey}`
      )
        .then((response) => {
          if (!response.ok) {
        ...
        .catch((error) => {
          alert(
            "Failed to connect to Google Calendar. Please check your API Key."
          );
          console.error("Error:", error);
        });
    } else {
      const confirmRedirect = confirm(
        "No API Key entered. Do you want to visit the Google Cloud Console to generate one?"
      );
      if (confirmRedirect) {
        window.open(
