...
from backend.database.mongo_connection import mailing_list_collection, database # Import database instead of subscriptions_collection
import logging
            ...
            if user_email:
                ...
                if not existing_user:  # Only subscribe if not already in the mailing list
                    ...
                    if result.inserted_id:
                        ...
                        subscription_result = database.Subscriptions.insert_one(subscription_data) # Use database.Subscriptions
                ...
                # Redirect or return error if email is not in session
                return jsonify({"success": False, "message": "User email not found in session."}), 400
        else:
            # Handle GET request if needed, maybe render a subscription page
            return render_template('subscribe/subscribe_page.html') # Example template
    else:
...
def unsubscribe():
    ...
    if user_id:
        ...
        if user_email:
            ...
            subscription_result = database.Subscriptions.delete_one({"user_email": user_email}) # Use database.Subscriptions
            ...
            if mailing_list_result.deleted_count > 0 or subscription_result.deleted_count > 0: # Changed AND to OR
                logger.info(f"User {user_email} unsubscribed successfully from mailing list and/or subscriptions.")
                # Set a flag in session indicating that the user has unsubscribed
                session["unsubscribed"] = True
                return render_template('subscribe/unsubscribed.html')  # Confirmation page
            else:
