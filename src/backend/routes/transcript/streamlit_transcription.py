...
def render_translate_and_download(section_key: str, label: str, filename: str):
    ...
    lang = st.selectbox(
        f"🌍 Translate {label} to:", languages, key=f"{section_key}_lang"
    )

    if st.button(f"Translate {label}", key=f"{section_key}_translate_btn"):
        try:
            response = requests.post(
                f"{API_BASE_URL}/translate", json={"text": content, "language": lang}
            )
    ...
    download_button_text(
        f"⬇ Download {label}", translated, filename, key_prefix=section_key
    )
...
with tab1:
            ...
            with st.spinner("🔄 Transcribing... Please wait."):
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        st.session_state.raw_transcription = result.get(
                            "raw_transcription", ""
                        )
                        st.session_state.full_transcript = result.get(
                            "full_transcript", ""
                        )
                        st.session_state.ai_suggestions = result.get(
                            "ai_suggestions", ""
                        )
    ...
    if st.session_state.get("raw_transcription", ""):
        ...
        raw_text = (
            st.session_state.get("raw_transcription_translated")
            or st.session_state.raw_transcription
        )
        st.text_area("Raw Transcription", value=raw_text, height=200)
        render_translate_and_download(
            "raw_transcription", "Raw Transcription", "raw_transcription.txt"
        )
        ...
        st.write(
            "Removes filler words and unnecessary expressions from your transcript."
        )
        if st.button("Generate Clean Transcript"):
            ...
            if response.status_code == 200:
                st.session_state.transcription_no_fillers = response.json().get(
                    "clean_transcript", ""
                )
        ...
        if (
            st.session_state.show_clean_transcript
            and st.session_state.transcription_no_fillers
        ):
            clean_text = (
                st.session_state.get("transcription_no_fillers_translated")
                or st.session_state.transcription_no_fillers
            )
            st.text_area("Clean Transcript", value=clean_text, height=200)
            render_translate_and_download(
                "transcription_no_fillers", "Clean Transcript", "clean_transcript.txt"
            )

        # 2) AI Suggestions
        st.markdown("### 🤖 AI Suggestions")
        st.write(
            "Get ideas or improvements for your transcript, e.g. better phrasing or structure."
        )
        if st.button("Generate AI Suggestions"):
            ...
            if response.status_code == 200:
                st.session_state.ai_suggestions = response.json().get(
                    "ai_suggestions", ""
                )
        ...
        if st.session_state.show_ai_suggestions and st.session_state.ai_suggestions:
            ai_text = (
                st.session_state.get("ai_suggestions_translated")
                or st.session_state.ai_suggestions
            )
            st.text_area("AI Suggestions", value=ai_text, height=200)
            render_translate_and_download(
                "ai_suggestions", "AI Suggestions", "ai_suggestions.txt"
            )

        # 3) Show Notes
        st.markdown("### 📝 Show Notes")
        st.write(
            "Automatically summarize the main points in the transcript for easy reference."
        )
        ...
        if st.session_state.show_show_notes and st.session_state.show_notes:
            notes_text = (
                st.session_state.get("show_notes_translated")
                or st.session_state.show_notes
            )
        ...
        if st.session_state.show_quotes and st.session_state.quotes:
            quotes_text = (
                st.session_state.get("quotes_translated") or st.session_state.quotes
            )
            ...
            if st.button("Generate Quote Images"):
                ...
                if response.status_code == 200:
                    st.session_state.quote_images = response.json().get(
                        "quote_images", []
                    )
            ...
            if st.session_state.show_quote_images and st.session_state.get(
                "quote_images", []
            ):
                ...
                for i, url in enumerate(st.session_state.quote_images, 1):
                    if url:
                        ...
                        st.markdown(
                            f"[⬇ Download Image {i}]({url})", unsafe_allow_html=True
                        )
...
with tab2:
        ...
        if st.button("Enhance Audio"):
            with st.spinner("🔄 Enhancing audio..."):
                ...
                try:
                    ...
                    response = requests.post(
                        f"{API_BASE_URL}/audio/enhancement", files=files
                    )

                    # Log the response status code
                    logger.info(
                        f"Response from /audio/enhancement: {response.status_code}"
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        if enhanced_audio_file_id:
                            ...
                            logger.info(
                                f"🆔 Fetching enhanced audio with ID: {enhanced_audio_file_id}"
                            )
                            print(
                                f"🆔 Fetching enhanced audio with ID: {enhanced_audio_file_id}"
                            )  # Print to console
                            ...
                            logger.info(
                                f"📡 Fetching enhanced audio from {API_BASE_URL}/get_file/{enhanced_audio_file_id}"
                            )

                            # Fetch the file
                            enhanced_audio_response = requests.get(
                                f"{API_BASE_URL}/get_file/{enhanced_audio_file_id}"
                            )

                            # Log the response status
                            logger.info(
                                f"📩 Response status: {enhanced_audio_response.status_code}"
                            )

                            # Check if response is OK
                            if enhanced_audio_response.status_code == 200:
                                logger.info(
                                    f"✅ File received! Size: {len(enhanced_audio_response.content)} bytes"
                                )
                                st.audio(
                                    enhanced_audio_response.content, format="audio/wav"
                                )

                                # Store in session state
                                st.session_state["enhanced_audio"] = (
                                    enhanced_audio_response.content
                                )
                                ...
                                logger.error(
                                    f"❌ Failed to fetch file. Response: {enhanced_audio_response.text}"
                                )
                                st.error(
                                    "❌ Failed to fetch the enhanced audio file. Please try again."
                                )

                        else:
                            logger.error(
                                "Processed audio file ID not found in the response."
                            )
                            st.error("❌ Processed audio file not found.")
                    else:
                        logger.error(
                            f"Error enhancing audio. Status code: {response.status_code}"
                        )
                    ...
                    st.error(
                        "❌ Error enhancing audio. Please check your network or try again."
                    )
    ...
    if "enhanced_audio" in st.session_state:
        ...
        if st.button("📊 Analyze"):
            with st.spinner("🔄 Analyzing..."):
                ...
                logger.info(
                    f"📝 Temporary file created for analysis: {temp_audio_file_path}"
                )

                # Send the file to the analysis API
                with open(temp_audio_file_path, "rb") as f:
                    response = requests.post(
                        f"{API_BASE_URL}/audio_analysis", files={"audio": f}
                    )
                ...
                if response.status_code == 200:
                    ...
                    if clarity_score and isinstance(clarity_score, str):
                        ...
                        if len(clarity_lines) >= 4:
                            ...
                            st.write(
                                f"**Readability (Flesch-Kincaid Score)**: {clarity_lines[2]}"
                            )
                    ...
                    try:
                        ...
                        flesch_kincaid_line = next(
                            (
                                line
                                for line in clarity_lines
                                if "Flesch-Kincaid" in line
                            ),
                            None,
                        )
                        if flesch_kincaid_line:
                            flesch_kincaid_score = float(
                                flesch_kincaid_line.split(": ")[1]
                            )
                        else:
                            flesch_kincaid_score = None
                    except (IndexError, ValueError, TypeError) as e:
                        st.warning(
                            f"⚠️ Could not extract Flesch-Kincaid Score. Error: {e}"
                        )
                        flesch_kincaid_score = None
                    ...
                    if flesch_kincaid_score is not None:
                        ...
                        st.write(
                            f"**Flesch-Kincaid Score Explanation**: A score of {flesch_kincaid_score} indicates that the text is {grade_level}."
                        )
                        st.write(f"**Example of Readability**: {example_text}")
                    st.write(
                        "**Filler Words Penalty**: A higher number of filler words results in a lower clarity score."
                    )
                    ...
                    st.write(
                        f"**Speech Rate**: The speech rate is calculated as the number of words spoken per minute (WPM)."
                    )
                    st.write(
                        f"A speech rate of **{speech_rate}** WPM means that the speaker spoke approximately **{speech_rate}** words every minute."
                    )
                    st.write("**How to interpret the number**:")
                    st.write(
                        "- **Below 100 WPM**: Slower pace, with more pauses between words. Might indicate deliberate speech or careful articulation."
                    )
                    st.write(
                        "- **100–130 WPM**: Typical for casual conversation. Common in everyday discussions."
                    )
                    st.write(
                        "- **130–160 WPM**: A bit faster pace, common in public speaking, presentations, or storytelling."
                    )
                    st.write(
                        "- **Above 160 WPM**: Indicates faster speech, possibly rushed or energetic. Often seen in fast talkers or during rapid discussions."
                    )
                    st.write(
                        "- **Ideal Range**: For comfortable understanding, a speech rate around **120–150 WPM** is usually considered ideal in conversation."
                    )
                    ...
                    st.error(
                        "❌ Error analyzing emotion, sentiment, clarity, or background noise."
                    )
    ...
    voice_file = st.file_uploader(
        "📂 Upload an audio file for voice isolation",
        type=["wav", "mp3"],
        key="voice_isolator",
    )
        ...
        if st.button("🎙️ Isolate Voice"):
            with st.spinner("🔄 Isolating voice using ElevenLabs..."):
                try:
                    ...
                    response = requests.post(
                        f"{API_BASE_URL}/voice_isolate", files=files
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(
                            f"📡 Fetching isolated voice file from: {fetch_url}"
                        )
                        ...
                        if isolated_response.status_code == 200:
                            ...
                            st.session_state["isolated_voice"] = (
                                isolated_response.content
                            )
    ...
    audio_file_cut = st.file_uploader(
        "📂 Upload an audio file for cutting",
        type=["wav", "mp3"],
        key="audio_uploader_cut",
    )
    ...
    if audio_file_cut:
        ...
        if "uploaded_audio_id" not in st.session_state:
            with st.spinner("🔄 Uploading file to MongoDB..."):
                ...
                if response.status_code == 200:
                    result = response.json()
                    st.session_state["uploaded_audio_id"] = result[
                        "audio_file_id"
                    ]  # Store in session state
                    st.session_state["waveform_file_id"] = result[
                        "waveform"
                    ]  # Store waveform ID
                    st.session_state["audio_duration"] = result[
                        "duration"
                    ]  # Store duration

                    print(
                        f"🆔 Stored audio file ID: {st.session_state['uploaded_audio_id']}"
                    )
                    print(
                        f"🆔 Stored waveform file ID: {st.session_state['waveform_file_id']}"
                    )
        ...
        if waveform_file_id:
            waveform_response = requests.get(
                f"{API_BASE_URL}/get_file/{waveform_file_id}"
            )
        ...
        start_time = st.slider(
            "Start Time (seconds)", 0.0, duration, 0.0, step=0.1, key="start_time_cut"
        )
        end_time = st.slider(
            "End Time (seconds)", 0.0, duration, duration, step=0.1, key="end_time_cut"
        )
        ...
        if st.button("✂ Cut Audio"):
            with st.spinner("🔄 Processing audio..."):
                ...
                data = {
                    ...
                    "clips": [{"start": start_time, "end": end_time}],
                }
                ...
                if response.status_code == 200:
                    ...
                    if clipped_audio_file_id:
                        ...
                        clipped_audio_response = requests.get(
                            f"{API_BASE_URL}/get_file/{clipped_audio_file_id}"
                        )

                        print(
                            f"📡 Clipped audio fetch status: {clipped_audio_response.status_code}"
                        )
    ...
    audio_file_cut_ai = st.file_uploader(
        "📂 Upload an audio file for AI cutting",
        type=["wav", "mp3"],
        key="audio_uploader_cut_ai",
    )
        ...
        if "file_id" not in st.session_state:
            with st.spinner("🔄 Uploading file to MongoDB..."):
                ...
                upload_response = requests.post(
                    f"{API_BASE_URL}/get_audio_info", files=files
                )

                if upload_response.status_code == 200:
                    upload_result = upload_response.json()
                    st.session_state.file_id = upload_result.get("audio_file_id")
                    st.session_state.waveform_id = upload_result.get(
                        "waveform"
                    )  # Store waveform file ID
                    st.session_state.duration = upload_result.get(
                        "duration", 60.0
                    )  # Default to 60s

                    st.text(
                        "🔊 Audio File Uploaded. Click 'Analyze Audio' to start AI processing."
                    )
                else:
            ...
            st.text(
                "✅ File already uploaded. Click 'Analyze Audio' to start AI processing."
            )
    ...
    if "file_id" in st.session_state:
        if st.button("Analyze Audio"):
            with st.spinner("🔄 Using AI to process audio..."):
                if "file_id" not in st.session_state:
                    st.error(
                        "❌ No audio file ID found. Please upload an audio file first."
                    )
                    st.stop()
                ...
                waveform_response = requests.get(
                    f"{API_BASE_URL}/get_file/{st.session_state.waveform_id}"
                )
                ...
                response = requests.post(
                    f"{API_BASE_URL}/ai_cut_audio",
                    json={"file_id": file_id, "clips": []},
                )

                if response.status_code == 200:
                    result = response.json()

                    st.session_state.cleaned_transcript = result.get(
                        "cleaned_transcript", "No transcript available."
                    )
                    st.session_state.background_noise = result.get(
                        "background_noise", "No noise detected."
                    )
                    st.session_state.suggested_cuts = result.get("suggested_cuts", [])
                    st.session_state.sentence_certainty_scores = result.get(
                        "sentence_certainty_scores", []
                    )
                    st.session_state.sentence_timestamps = result.get(
                        "sentence_timestamps", []
                    )
                    st.session_state.long_pauses = result.get("long_pauses", [])
                    st.session_state.selected_sentences = (
                        []
                    )  # Initialize bulk selection list
                    st.session_state.removed_sentences = (
                        []
                    )  # Store removed sentences for Undo Last
                    st.session_state.original_transcript = result.get(
                        "sentence_certainty_scores", []
                    )[
                        :
                    ]  # Store Original State for Undo All

                    # ✅ Add AI Sentiment Analysis & Show Notes
                    st.session_state.ai_sentiment = result.get(
                        "sentiment", "Unknown"
                    )  # Store sentiment
                    st.session_state.ai_show_notes = result.get(
                        "ai_show_notes", "No notes available."
                    )  # Store AI-generated show notes
    ...
    if st.session_state.get("analyzed", False):
        ...
        st.write(
            f"**Overall Sentiment:** {st.session_state.get('ai_sentiment', 'Unknown')}"
        )

        # Display background noise result
        st.write("🔊 **Background Noise Detection**", unsafe_allow_html=True)
        st.text(
            st.session_state.background_noise
            if "background_noise" in st.session_state
            else "No noise detected."
        )
        ...
        if st.session_state.suggested_cuts:
            for cut in st.session_state.suggested_cuts:
                ...
                st.write(
                    f"**Certainty Level:** {cut['certainty_level']} (Start: {cut['start']}s, End: {cut['end']}s)"
                )
        else:
            st.text("✅ No suggested cuts found.")

        # **Toggle for full transcript view**
        full_transcript_view = st.checkbox(
            "Show Full Transcript (Including Low Certainty)", value=False
        )

        # **Filter transcript by 60% certainty level by default**
        displayed_sentences = (
            st.session_state.sentence_certainty_scores
            if full_transcript_view
            else [
                entry
                for entry in st.session_state.sentence_certainty_scores
        ...
        for idx, entry in enumerate(displayed_sentences):
            ...
            new_sentence = st.text_input(
                f"Edit Sentence {idx + 1}", entry["sentence"], key=f"edit_{idx}"
            )
            entry["sentence"] = new_sentence  # Update stored sentence

            st.write(
                f"**Certainty Level:** {entry['certainty_level']} (Score: {entry['certainty']})"
            )
            ...
            timestamp = next(
                (
                    s
                    for s in st.session_state.sentence_timestamps
                    if s["id"] == sentence_id
                ),
                None,
            )
            ...
            if timestamp:
                ...
                if st.button(f"▶ Play {entry['sentence'][:10]}...", key=f"play_{idx}"):
                    if "file_id" in st.session_state:
                        ...
                        audio_response = requests.get(
                            f"{API_BASE_URL}/get_file/{file_id}"
                        )
                ...
                if st.button(
                    f"🔙 Rewind 5s Before {entry['sentence'][:10]}...",
                    key=f"rewind_{idx}",
                ):
        ...
        **Magic Cut Threshold** is an AI-powered editing tool that removes sentences based on a **certainty score** (0-100%).
        ...
        st.session_state.certainty_threshold = st.slider(
            "Magic Cut Threshold", 0.0, 1.0, 0.6, step=0.1
        )
        ...
        if st.button("✂ Magic Cut Sentences Above Threshold", key="magic_cut_button"):
            ...
            st.session_state.sentence_certainty_scores = [
                ...
                for entry in st.session_state.sentence_certainty_scores
                if entry["certainty"]
                < st.session_state.certainty_threshold  # ✅ Corrected filtering
            ]
            st.session_state.selected_sentences = []  # Clear selection
            st.rerun()
        ...
        cleaned_sentences = [
            entry["sentence"] for entry in st.session_state.sentence_certainty_scores
        ]
        ...
        st.download_button(
            ...
            file_name="final_transcript.txt",
            mime="text/plain",
        )
        ...
        start_time = st.slider(
            "Start Time (seconds)", 0.0, st.session_state.duration, 0.0, step=0.1
        )
        end_time = st.slider(
            "End Time (seconds)",
            0.0,
...
with tab3:
    ...
    video_file = st.file_uploader(
        "📂 Upload a video file", type=["mp4", "mov", "mkv"], key="video_uploader"
    )
    if video_file:
        st.video(video_file)
        ...
        if "video_id" not in st.session_state:
            with st.spinner("🔄 Uploading video to MongoDB..."):
                ...
                upload_response = requests.post(
                    f"{API_BASE_URL}/ai_videoedit", files=files
                )
                if upload_response.status_code == 200:
                    upload_result = upload_response.json()
                    st.session_state["video_id"] = upload_result.get("video_id")
                    st.text(
                        "✅ Video Uploaded! Click 'Enhance Video' to start processing."
                    )
                else:
            ...
            st.text(
                "✅ Video already uploaded. Click 'Enhance Video' to start processing."
            )
        ...
        if st.button("Enhance Video"):
            with st.spinner("🔄 Enhancing video..."):
                ...
                response = requests.post(
                    f"{API_BASE_URL}/ai_videoenhance", json={"video_id": video_id}
                )
                if response.status_code == 200:
                    ...
                    if processed_video_id:
                        ...
                        processed_video_url = (
                            f"{API_BASE_URL}/get_video/{processed_video_id}"
                        )
                        st.video(processed_video_url)
                        st.markdown(
                            f"[📥 Download Enhanced Video]({processed_video_url})",
                            unsafe_allow_html=True,
                        )
                    else:
    ...
    if "processed_video_id" in st.session_state:
        ...
        if st.button("Analyze Video"):
            with st.spinner("🔄 Analyzing video..."):
                ...
                response = requests.post(
                    f"{API_BASE_URL}/ai_videoanalysis", json={"video_id": video_id}
                )
                if response.status_code == 200:
                    ...
                    st.write("📜 **Transcript:**")
                    st.write(
                        analysis_results.get("transcript", "No transcript available")
                    )

                    st.write("🔊 **Background Noise Detection:**")
                    st.write(
                        analysis_results.get("background_noise", "No data available")
                    )

                    st.write("🗣 **Sentiment Analysis:**")
                    st.write(
                        analysis_results.get("sentiment", "No sentiment available")
                    )
    ...
    video_file_cut = st.file_uploader(
        "📂 Upload a video file for cutting",
        type=["mp4", "mov", "mkv"],
        key="video_uploader_cut",
    )
    if video_file_cut:
        ...
        if "uploaded_video_id" not in st.session_state:
            with st.spinner("🔄 Uploading video to MongoDB..."):
                ...
                if upload_response.status_code == 200:
                    upload_result = upload_response.json()
                    st.session_state["uploaded_video_id"] = upload_result.get(
                        "video_id"
                    )
        ...
        if video_id:
            ...
            duration = st.number_input(
                "Enter total duration of video (seconds)", min_value=1.0, step=0.1
            )
            ...
            if st.button("✂ Cut Video"):
                with st.spinner("🔄 Processing video..."):
                    data = {
                        "video_id": video_id,
                        "clips": [{"start": start_time_video, "end": end_time_video}],
                    }
                    ...
                    if response.status_code == 200:
                        ...
                        if clipped_video_id:
                            ...
                            clipped_video_url = (
                                f"{API_BASE_URL}/get_video/{clipped_video_id}"
                            )
                            st.video(clipped_video_url)
                            st.markdown(
                                f"[📥 Download Clipped Video]({clipped_video_url})",
                                unsafe_allow_html=True,
                            )
                        else:
                            st.error("❌ Error: Clipped file ID not found.")
