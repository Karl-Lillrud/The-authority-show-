            ...
            if certainty <= 0.2
                ...
                if certainty <= 0.4
                    ...
                    if certainty <= 0.6
                        ...
                        "Orange"
                        if certainty <= 0.8
                        else "Dark Orange" if certainty <= 0.9 else "Red"
                    )
                )
            )
        )
        result_list.append(
            {"sentence": s, "certainty": certainty, "certainty_level": level}
        )
    return result_list
...
def get_sentence_timestamps(sentence, word_timings, prev_end_time=0):
    ...
    start = next(
        (
            ...
            if w["word"] == first_word and w["start"] >= prev_end_time
        ),
        prev_end_time,
    )
    end = next(
        (
            w["end"]
            for w in word_timings
            if w["word"] == last_word and w["end"] >= start
        ),
        start + 2.0,
    )
    ...
    try:
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You're an expert podcast editor and copywriter.",
                },
                {"role": "user", "content": prompt},
            ],
        )
