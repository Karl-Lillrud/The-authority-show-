    ...
    try:
        ...
        if response.status_code == 201:
            ...
            try:
                ...
                logger.error(
                    f"Error submitting RSS feed to Spotify: {response.status_code} - {error_details}"
                )
            except ValueError:
                # Log the raw response body if it is not JSON
                logger.error(
                    f"Error submitting RSS feed to Spotify: {response.status_code} - Raw response: {response.text}"
                )
            return False
    except Exception as e:
        logger.error(
            f"Exception occurred while submitting RSS feed to Spotify: {e}",
            exc_info=True,
        )
        return False
