...
class SubscriptionService:
    def __init__(self):
        ...
        )  # Changed from subscriptions_collection
        ...
        try:
            ...
            try:
                ...
                existing_subscriptions = list(
                    self.subscriptions_collection.find(
                        {"user_id": user_id, "status": "active"}
                    )
                )

                logger.info(
                    f"Found {len(existing_subscriptions)} active subscriptions in database for user {user_id}"
                )

                # Process each subscription from our database
                for existing_sub in existing_subscriptions:
                    if (
                        existing_sub.get("payment_id")
                        and existing_sub.get("payment_id") != current_session_id
                    ):
                        ...
                        if payment_id.startswith("sub_"):
                            logger.info(
                                f"Found existing Stripe subscription ID: {payment_id} for user {user_id}"
                            )
                        ...
                        # If payment_id is a checkout session, try to get subscription from it
                            ...
                            try:
                                ...
                                if checkout_session and hasattr(
                                    ...
                                    if stripe_subscription_id:
                                        logger.info(
                                            f"Retrieved subscription ID {stripe_subscription_id} from session {payment_id}"
                                        )
                                ...
                                logger.error(
                                    f"Error retrieving subscription from session {payment_id}: {session_err}"
                                )
                ...
                if account.get("email"):
                    try:
                        ...
                        customers = stripe.Customer.list(
                            email=account.get("email"), limit=5
                        )
                        if customers and customers.data:
                            for customer in customers.data:
                                logger.info(
                                    f"Found Stripe customer by email: {customer.id}"
                                )
                                ...
                                if customer_subs and customer_subs.data:
                                    for sub in customer_subs.data:
                                        ...
                                        if (
                                            hasattr(stripe_session, "subscription")
                                            and stripe_session.subscription == sub.id
                                        ):
                                            logger.info(
                                                f"Skipping current subscription {sub.id} that's being created"
                                            )
                                            continue

                                        logger.info(
                                            f"Found active subscription {sub.id} for customer {customer.id}"
                                        )
                                        found_subscriptions.append(sub.id)
                    except Exception as customer_err:
                        logger.error(
                            f"Error finding subscriptions by customer email: {customer_err}"
                        )
                    ...
                    try:
                        ...
                        if customer_subs and customer_subs.data:
                            for sub in customer_subs.data:
                                ...
                                logger.info(
                                    f"Found active subscription {sub.id} for customer ID {customer_id}"
                                )
                                found_subscriptions.append(sub.id)
                    except Exception as direct_customer_err:
                        logger.error(
                            f"Error finding subscriptions by direct customer ID: {direct_customer_err}"
                        )
                ...
                logger.info(
                    f"Attempting to cancel {len(found_subscriptions)} subscriptions: {found_subscriptions}"
                )
                ...
                for subscription_id in found_subscriptions:
                    try:
                        ...
                        logger.info(
                            f"Subscription {subscription_id} status: {sub_check.status}, cancel_at_period_end: {sub_check.cancel_at_period_end}"
                        )

                        if (
                            sub_check.status == "active"
                            and not sub_check.cancel_at_period_end
                        ):
                            ...
                            logger.info(
                                f"Successfully canceled subscription {subscription_id} at period end"
                            )
                            ...
                            try:
                                self.activity_service.log_activity(
                                    user_id=user_id,
                                    activity_type="subscription_cancellation_requested",
                                    description=f"Subscription {subscription_id} set to cancel at period end.",
                                    details={
                                        "subscription_id": subscription_id,
                                        "reason": "User purchased a new plan",
                                        "new_plan": plan_name,
                                    },
                                )
                            except Exception as log_e:
                                logger.error(
                                    f"Failed to log subscription_cancellation_requested activity: {log_e}"
                                )
                        ...
                        logger.error(
                            f"Error canceling subscription {subscription_id}: {cancel_err}"
                        )

            except Exception as cancel_process_err:
                logger.error(
                    f"Error during subscription cancellation process: {cancel_process_err}"
                )
            ...
            try:
                ...
                logger.info(
                    f"Marked {update_inactive_result.modified_count} previous active subscriptions as inactive for user {user_id}"
                )
            except Exception as inactive_err:
                logger.error(
                    f"Error marking previous subscriptions as inactive for user {inactive_err}",
                    exc_info=True,
                )
            ...
            try:
                ...
                logger.info(
                    f"Updated credits for user {user_id} to {plan_benefits.get('credits', 0)} credits based on {plan_name} plan"
                )
                ...
                logger.error(
                    f"Failed to update subscription credits: {credit_err}",
                    exc_info=True,
                )
                return True, None
    ...
    def update_subscription_from_webhook(self, user_id, plan_name, stripe_subscription):
        ...
        try:
            ...
            logger.error(
                f"Error updating subscription from webhook for user {user_id}: {str(e)}"
            )
            raise Exception(f"Error updating subscription: {str(e)}")
    ...
    def handle_subscription_renewal(self, user_id, stripe_subscription):
        ...
        try:
            ...
            if account.get("subscriptionEnd"):
                try:
                    ...
                    current_end_date = datetime.fromisoformat(
                        account["subscriptionEnd"].replace("Z", "+00:00")
                    )
                    ...
                    new_end_date = current_end_date + timedelta(
                        days=30
                    )  # Add 30 days for the next cycle

                    logger.info(
                        f"Renewing subscription from {current_end_date.isoformat()} to {new_end_date.isoformat()}"
                    )
                except (ValueError, AttributeError) as e:
                    logger.error(
                        f"Error parsing current subscription end date: {str(e)}"
                    )
                    # Fall back to Stripe dates if there's an error
                    new_start_date = datetime.fromtimestamp(
                        stripe_subscription.current_period_start, tz=timezone.utc
                    )
            ...
            plan_name = (
                current_plan.upper() if isinstance(current_plan, str) else "FREE"
            )
            amount_paid = 0

            # Try to extract plan information from Stripe only if needed
            try:
                if hasattr(stripe_subscription, "items") and hasattr(
                    stripe_subscription.items, "data"
                ):
                    for item in stripe_subscription.items.data:
                        if hasattr(item, "price") and hasattr(
                            ...
                            try:
                                if hasattr(item.price, "product"):
                                    ...
                                    extracted_plan = (
                                        product.metadata.get("plan") or product.name
                                    )
                                    if extracted_plan:
                                        logger.info(
                                            f"Extracted plan from Stripe product: {extracted_plan}"
                                        )
                                ...
                                logger.error(
                                    f"Error retrieving plan name from product: {str(e)}"
                                )
                                # Continue with current plan if extraction fails
                else:
                    # Try alternative method to get subscription data
                    logger.info(
                        "Items data not available on subscription object, trying direct lookup"
                    )
                    ...
                    if hasattr(stripe_subscription, "id"):
                        try:
                            ...
                            if hasattr(full_subscription, "items") and hasattr(
                                full_subscription.items, "data"
                            ):
                                for item in full_subscription.items.data:
                                    if hasattr(item, "price"):
                                        amount_paid += item.price.unit_amount / 100
                                        if (
                                            hasattr(item.price, "product")
                                            and item.price.product
                                        ):
                                            extracted_plan = (
                                                item.price.product.metadata.get("plan")
                                                or item.price.product.name
                                            )
                                            if extracted_plan:
                                                logger.info(
                                                    f"Extracted plan from full subscription: {extracted_plan}"
                                                )
                                                # Only override current plan if we successfully extracted one
                                                if (
                                                    extracted_plan.upper()
                                                    in PLAN_BENEFITS
                                                ):
                                                    plan_name = extracted_plan.upper()
                                                elif "PRO" in extracted_plan.upper():
                                                    ...
                                                    "ENTERPRISE"
                                                    in extracted_plan.upper()
                                                ):
                                                    plan_name = "ENTERPRISE"
                        except Exception as e:
                            logger.error(
                                f"Error retrieving full subscription: {str(e)}"
                            )
                            # Continue with current plan if extraction fails
            except Exception as e:
            ...
            if plan_name not in PLAN_BENEFITS:
                logger.warning(
                    f"Plan name '{plan_name}' not found in PLAN_BENEFITS, using current plan '{current_plan}'"
                )
                plan_name = (
                    current_plan.upper() if isinstance(current_plan, str) else "FREE"
                )
            ...
            if update_result.modified_count == 0:
                logger.warning(
                    f"No changes made to account for user {user_id} during renewal"
                )
            ...
            logger.info(
                f"Successfully processed subscription renewal for user {user_id}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Error handling subscription renewal for user {user_id}: {str(e)}",
                exc_info=True,
            )
    ...
    def can_create_episode(self, user_id, is_imported=False):
        try:
            ...
            if is_imported:
                logger.info(
                    f"🔁 Skipping episode slot check for imported episode by user {user_id}"
                )
            ...
            logger.info(
                f"📊 Found {current_count} regular (non-imported) episodes for user {user_id}"
            )
            logger.info(
                f"📊 User has {base_slots} base slots and {extra_slots} extra slots (total: {total_allowed_slots})"
            )

            if current_count < total_allowed_slots:
                logger.info(
                    f"✅ User {user_id} is within allowed limit: {current_count}/{total_allowed_slots}"
                )
                ...
                if extra_slots > 0:
                    return (
                        False,
                        f"You've used your {base_slots} base slots and all {extra_slots} extra slot(s).",
                    )
                else:
                    return (
                        False,
                        f"You've used your {base_slots} free episode slots. Upgrade to unlock more.",
                    )

        except Exception as e:
            logger.error(
                f"❌ Error checking create-episode permission for user {user_id}: {str(e)}"
            )
    ...
    def get_episode_slot_count(self, user_id, subscription):
        ...
        try:
            now = datetime.utcnow()
            start = (
                parse_date(subscription["start_date"])
                if subscription.get("start_date")
                else now - timedelta(days=30)
            )
            end = (
                parse_date(subscription["end_date"])
            ...
            count = self.episodes_collection.count_documents(
                {
                    ...
                    "$or": [{"isImported": {"$exists": False}}, {"isImported": False}],
                }
            )
