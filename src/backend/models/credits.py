...
class CreditHistoryEntrySchema(Schema):
    ...
    type = fields.Str(required=True, validate=validate.OneOf(
        ["initial_pm", "initial_user", "purchase", "monthly_pm_grant", "consumption", "pm_reset", "user_reset", "adjustment"]
    ))
    amount = fields.Int(required=True) # Can be positive (grant/purchase) or negative (consumption/reset)
    description = fields.Str(required=True)
    balance_after = fields.Dict(keys=fields.Str(), values=fields.Int(), required=False) # Optional: store pm/user balance after tx
