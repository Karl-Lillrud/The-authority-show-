...
class EpisodeRepository:
    ...
    def register_episode(self, data, user_id):
        ...
        try:
            ...
            if files:
                ...
                if not saved_files:
                    logger.error("No files were saved to Google Cloud Storage.")
            ...
            result = self.collection.insert_one(episode_doc)
    ...
    def get_episode(self, episode_id, user_id):
        ...
        try:
            ...
            logger.info(
                f"Fetching episode with ID: {episode_id} for user ID: {user_id_str}"
            )
            ...
            if not episode:
                logger.warning(
                    f"Episode with ID {episode_id} not found for user ID {user_id_str}."
                )
                # Log additional debug information
                logger.debug(
                    f"Available episodes for user ID {user_id_str}: {list(self.collection.find({'userid': user_id_str}, {'_id': 1}))}"
                )
    ...
    def update_episode(self, episode_id, user_id, data):
        ...
        try:
            logger.info(
                f"Updating episode with ID: {episode_id} for user ID: {user_id}"
            )
            ep = self.collection.find_one({"_id": episode_id})
            if not ep:
            ...
            if ep["userid"] != str(user_id):
                logger.warning(
                    f"Permission denied for user ID {user_id} to update episode ID {episode_id}."
                )
                return {"error": "Permission denied"}, 403

            update_fields = {
                "title": (
                    data.get("title", ep["title"]).strip()
                    if data.get("title")
                    else ep["title"]
                ),
                "description": (
                    data.get("description", ep["description"]).strip()
                ...
                "status": (
                    data.get("status", ep["status"]).strip()
                    if data.get("status")
                    else ep["status"]
                ),
                "audioUrl": data.get(
    ...
    def get_episodes_by_podcast(self, podcast_id, user_id, return_with_status=False):
        ...
        try:
            logger.info(
                f"Fetching episodes for podcast ID: {podcast_id} and user ID: {user_id}"
            )
            episodes = list(
            ...
            if not episodes:
                ...
                logger.info(
                    f"Found {len(episodes)} episodes for podcast ID {podcast_id}."
                )
