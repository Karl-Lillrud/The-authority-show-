...
class TeamInviteRepository:
    ...
    def save_invite(self, team_id, email, inviter_id):
        ...
        existing_invite = self.invites_collection.find_one({
            ...
            "status": "pending",
            "createdAt": {"$gt": datetime.now(timezone.utc) - timedelta(hours=24)}  # Expire after 24 hours
        })

        if existing_invite:
            logger.info(f"Found existing active invite for {normalized_email} to team {team_id}")
        ...
        logger.info(f"Created new invite {invite_token} for {normalized_email} to team {team_id}")
    ...
    def cancel_invite(self, invite_token, user_id):
        ...
        if invite["status"] == "expired" or (expires_at and expires_at < datetime.now(timezone.utc)):
            self.invites_collection.update_one(
                {"_id": invite_token},
