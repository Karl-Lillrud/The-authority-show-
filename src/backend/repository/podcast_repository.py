...
class PodcastRepository:
    ...
    def add_podcast(self, user_id, data):
        try:
            ...
            if not user_account:
                ...
                account_count = collection.database.Accounts.count_documents({"ownerId": user_id})
                logger.error(f"Total accounts found for ownerId {user_id}: {account_count}")
                raise ValueError("No account associated with this user (owner)")

            logger.info(f"Found account for ownerId {user_id}: Account _id: {user_account.get('_id')}")
            ...
            account = collection.database.Accounts.find_one({"_id": account_id, "ownerId": user_id})
            if not account:
                logger.error(f"Consistency check failed: Account _id {account_id} not found or doesn't belong to owner {user_id}")
            ...
            if result.inserted_id:
                try:
                    ...
                    logger.error(f"Failed to log podcast_created activity: {act_err}", exc_info=True)
                return {
            ...
            logger.error(f"General Exception in add_podcast for user {user_id}: {e}", exc_info=True)
            return {"error": "Failed to add podcast", "details": str(e)}, 500

    def get_podcasts(self, user_id):
        try:
            user_accounts = list(collection.database.Accounts.find({"ownerId": user_id}, {"_id": 1}))
    ...
    def get_podcast_by_id(self, user_id, podcast_id):
        try:
            user_accounts = list(collection.database.Accounts.find({"ownerId": user_id}, {"id": 1, "_id": 1}))
        ...
        try:
            ...
            if result.deleted_count == 1:
                try:
                    ...
                    logger.error(f"Failed to log podcast_deleted activity: {act_err}", exc_info=True)
    ...
    def fetch_rss_feed(self, rss_url):
        try:
            ...
            logger.error("❌ ERROR fetching RSS feed via PodcastRepository: %s", e, exc_info=True)
    ...
    def addPodcastWithRss(self, user_id, rss_url):
        ...
        try:
            ...
            # Fetch user account to get accountId
            user_account = collection.database.Accounts.find_one({"ownerId": user_id})
            if not user_account:
                logger.error(f"Account lookup failed for ownerId: {user_id}")
                raise ValueError("No account associated with this user (owner)")
            ...
            # Prepare full podcast data for validation & insert
            podcast_data_for_add = {
                ...
                "ownerName": "",  # set sensible defaults or extract if possible
                "hostName": "",
                "googleCal": "",
                "podUrl": "",
                "guestUrl": "",
            ...
            # Validate and add podcast
            response, status = self.add_podcast(user_id, podcast_data_for_add)

            # If added successfully and rssImage exists, update that field
            if status == 201 and rss_data.get("imageUrl"):
                podcast_id = response.get("podcast_id")
                self.collection.update_one(
                    {"_id": podcast_id},
                    {"$set": {"rssImage": rss_data["imageUrl"]}}
                )
            ...
            logger.error(f"ValueError in addPodcastWithRss for user {user_id}: {e}")
            if isinstance(e.args[0], str):
                return {"error": e.args[0]}, 400
            else:
                return {"error": e.args[0], "details": e.args[1]}, 400
            ...
            logger.error(f"Error in addPodcastWithRss: {e}", exc_info=True)
