        ...
        for (const episode of episodes) {
          ...
          try {
            const registerResponse = await registerEpisode({
              ...
              isImported: true // Add this line to mark episodes as imported
            });
      ...
      rssData.socialMedia && rssData.socialMedia.length > 0
        ? rssData.socialMedia
            ...
            .map((social) => {
              ...
              <a href="${
                social.url
              }" target="_blank" class="social-link ${platform}" rel="noreferrer">
                <i class="${icon}"></i>
  ...
  async function initWelcomePopup() {
    try {
      ...
      if (user.isFirstLogin) {
        ...
        const closeWelcomePopup = document.getElementById(
          "close-welcome-popup"
        );
...
function connectGoogleCalendar() {
  ...
  if (connectCalendarButton) {
    ...
    connectCalendarButton.innerHTML =
