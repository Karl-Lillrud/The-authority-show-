document.addEventListener('DOMContentLoaded', function() {
    ...
    // === Modal Elements for Team Invite (Add New Team) ===
    const addTeamBtn = document.getElementById('addTeamBtn');
    const teamInviteModal = document.getElementById('teamInviteModal');
    const closeTeamInviteModalBtn = document.getElementById('closeTeamInviteModal');
    const modalOverlay = document.getElementById('modalOverlay');
  
    if (addTeamBtn && teamInviteModal && modalOverlay) {
      addTeamBtn.addEventListener('click', function() {
        teamInviteModal.classList.add('show');
        teamInviteModal.setAttribute('aria-hidden', 'false');
        modalOverlay.classList.add('show');
      });
    }
    if (closeTeamInviteModalBtn && teamInviteModal && modalOverlay) {
      closeTeamInviteModalBtn.addEventListener('click', function() {
        teamInviteModal.classList.remove('show');
        teamInviteModal.setAttribute('aria-hidden', 'true');
        modalOverlay.classList.remove('show');
      });
    }
  
    // === Handle Team Invite Modal Form Submission ===
    const teamInviteForm = document.getElementById('teamInviteForm');
    if (teamInviteForm) {
      teamInviteForm.addEventListener('submit', async function(e) {
        e.preventDefault();
        const payload = {
          name: document.getElementById('teamName').value,
          email: document.getElementById('teamEmail').value,
          description: document.getElementById('teamDescription').value,
          isActive: document.getElementById('teamIsActive').checked,
          joinedAt: new Date().toISOString(),
          lastActive: new Date().toISOString(),
          members: []
        };
        try {
          const data = await addTeamRequest(payload);
          alert(data.message || "Team created successfully!");
          teamInviteForm.reset();
          teamInviteModal.classList.remove('show');
          teamInviteModal.setAttribute('aria-hidden', 'true');
          modalOverlay.classList.remove('show');
          // Optionally, refresh team dashboard if needed.
        } catch (err) {
    ...
    // Modal Elements for Adding/Editing Team Members (existing modals)
    const addMemberBtn = document.querySelector('.add-member');
    const addMemberModal = document.getElementById('addMemberModal');
    const addCloseModalBtn = addMemberModal ? addMemberModal.querySelector('.close-btn') : null;
    const editModal = document.getElementById('editModal');
    const editMemberForm = document.getElementById('editMemberForm');
    let currentEditingRow = null;
  
    if (addMemberBtn && addMemberModal && modalOverlay) {
    ...
    // === Add New Team Member on Dashboard ===
    const dashboardAddMemberForm = document.getElementById('addMemberForm');
    if (dashboardAddMemberForm) {
      dashboardAddMemberForm.addEventListener('submit', function(event) {
    ...
    function fetchTeamMembers() {
        fetch('/get_teams')
          ...
          .then(data => {
            if (data.length > 0) {
              const team = data[0]; // assuming a single team for the dashboard
              const tbody = document.querySelector('.team-table tbody');
              tbody.innerHTML = '';
              team.members.forEach(member => {
                const newRow = document.createElement('tr');
                newRow.setAttribute('data-id', member.userId); // or the unique identifier for the member
                newRow.innerHTML = `
                  <td>${member.name}</td>
                  <td>${member.role}</td>
    ...
    // === Fetch and Display Team Name in Table Header Row ===
    async function fetchTeamName() {
      try {
        ...
        if (teams.length > 0) {
          ...
          if (teamNameRow) {
            // Update the row with a single header cell spanning all columns (assumed 7 columns)
            teamNameRow.innerHTML = `<th colspan="7">${team.name}</th>`;
          }
        }
      } catch (err) {
    ...
    function validateName(nameValue) {
      ...
      if (valid) {
        errorEl && errorEl.classList.add('sr-only');
      } else {
        if (errorEl) {
