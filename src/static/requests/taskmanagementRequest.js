...
async function selectGuest(guestId, guestName, guestImage) {
  document.getElementById("selected-guest").textContent = guestName;
  document.getElementById("selected-guest-image").src = guestImage;
  document.getElementById("selected-guest-image").style.display = "block";
    ...
    if (response.ok) {
      ...
      data.episodes.forEach((episode) => {
        ...
        a.onclick = (event) => {
          ...
          selectEpisodeForView(episode.episode_id, episode.title);
        };
        ...
        b.onclick = (event) => {
          event.preventDefault();
          selectEpisode(episode.episode_id, guestId);
        };
...
function selectEpisode(episodeId, guestId) {
  console.log("Selected episode ID:", episodeId);
  addTasksToEpisode(episodeId, guestId);
}

async function viewEpisodeTasks(episodeId, episodeTitle) {
  try {
    ...
    if (response.ok) {
      ...
      // Update the header text to display the selected episode's title
      const header = document.querySelector(".current-tasks-header");
      header.textContent = `${episodeTitle} Tasks`;

      // Hide the "Add Tasks to Episode" button
      document.querySelector(".add-tasks-dropdown").style.display = "none";

      // Change the "Dashboard" button to "Back"
      const dashboardButton = document.getElementById("dashboard-button");
      dashboardButton.innerHTML = "&#8592; Pod Tasks";
      dashboardButton.href = "#";
      dashboardButton.onclick = (event) => {
        event.preventDefault();
        goBack();
      };
      ...
      data.tasks.forEach((task) => {
        ...
        li.innerHTML = `
            ...
            <div class="task-actions">
                ...
                <button class="delete-btn" onclick="deleteTask('${task._id}', true, '${episodeId}')">Delete</button>
            </div>
        `;
        taskList.appendChild(li);
      });

      console.log("Tasks fetched and displayed:", data.tasks);
    } else {
...
function selectEpisodeForView(episodeId, episodeTitle) {
  console.log("Selected episode ID for view:", episodeId);
  viewEpisodeTasks(episodeId, episodeTitle);
}

function goBack() {
  // Show the "Add Tasks to Episode" button
  document.querySelector(".add-tasks-dropdown").style.display = "inline-block";

  // Reset the header text
  const header = document.querySelector(".current-tasks-header");
  header.textContent = "Pod Tasks";

  // Restore the initial tasks
  const taskList = document.getElementById("task-list");
  taskList.innerHTML = "";
  ...
  document
    .querySelector(".current-tasks-header")
    .scrollIntoView({ behavior: "smooth" });

  // Change the "Back" button to "Dashboard"
  const dashboardButton = document.getElementById("dashboard-button");
  dashboardButton.innerHTML = "&#8592; Dashboard";
  dashboardButton.href = "{{ url_for('dashboard_bp.dashboard') }}";
  dashboardButton.onclick = null;
}
...
async function deleteTask(taskId, isEpisodeTask = false, episodeId = null) {
  try {
    const endpoint = isEpisodeTask
      ? `/episodes/delete_episode_task/${taskId}`
      : `/delete_podtask/${taskId}`;
    ...
    if (response.ok) {
      ...
      if (isEpisodeTask && episodeId) {
        viewEpisodeTasks(episodeId); // Refresh tasks for the episode
      } else {
...
async function fetchTasks() {
  try {
    ...
    if (response.ok) {
      ...
      initialTasks = data.podtasks; // Store the initial tasks
...
async function addTasksToEpisode(episodeId, guestId) {
  console.log("Adding tasks to episode ID:", episodeId);
  const taskList = document.getElementById("task-list");
  const tasks = Array.from(taskList.children).map((li) => ({
    taskname: li.querySelector(".task-name").textContent,
