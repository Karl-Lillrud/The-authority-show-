...
export function setupWebRTC(socket, config) {
    ...
    if (!pc) {
        console.log(`🆕 No peer connection found for ${data.fromUserId}, creating new`);
        ...
        peerConnections.set(data.fromUserId, pc);

        pc.onicecandidate = (event) => {
            if (event.candidate) {
                console.log(`📤 Sending ICE candidate to ${data.fromUserId}`);
                ...
                console.log(`🏁 ICE gathering complete for ${data.fromUserId}`);
            }
        };

        pc.ontrack = (event) => {
            console.log(`🎵 Received remote track from ${data.fromUserId}:`, event.streams);
            const remoteVideo = document.getElementById('remoteVideo');
            if (remoteVideo) {
                remoteVideo.srcObject = event.streams[0];
                remoteVideo.play().catch(err => console.warn('⚠️ Auto-play issue:', err));
            }
        };
    }
    ...
    pc.ontrack = async (event) => {
        ...
        if (event.streams[0]) {
            ...
            if (!connectedUsers.includes(userId)) {
                ...
                console.log(`👥 Added ${userId} to connectedUsers`);
            }
            ...
            if (videoElement) {
                ...
                try {
                    ...
                    console.log(`▶️ Video playback started for ${userId}`);
                } catch (err) {
                    console.warn(`⚠️ Could not autoplay video for ${userId}:`, err);
                }
