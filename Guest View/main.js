  ...
  if (guestListEl) {
    function renderGuestList(filterText = "") {
      ...
      guests
        .filter(guest => {
          let areasText = "";
          if (guest.areasOfInterest) {
            areasText = guest.areasOfInterest.join(" ").toLowerCase();
          }
          return (
            guest.name.toLowerCase().includes(searchLower) ||
            guest.description.toLowerCase().includes(searchLower) ||
            guest.tags.join(" ").toLowerCase().includes(searchLower) ||
            areasText.includes(searchLower)
          );
        })
        .forEach(guest => {
    ...
    if (popupFormEl) {
      popupFormEl.addEventListener("submit", function(e) {
        ...
        guests.push({ id, name, image, tags, description, areasOfInterest, email, linkedin, twitter, profileLink, status: "scheduled", scheduled: "0", completed: "0", bio: description });
        saveGuests(guests);
...
// Updated code with a default value
...
if (areasContainer) {
  ...
  areasContainer.textContent = areas.length ? "Areas of Interest: " + areas.join(", ") : "";
    ...
    if(status === "pending") { // if any pending exists, default to scheduled.
      status = "scheduled";
    }

// With a safer version that checks for undefined:
...
statusBadge.textContent = updatedStatus.charAt(0).toUpperCase() + updatedStatus.slice(1);
    ...
    function openEditProfilePopup() {
      ...
      // Do NOT add the schedule/completed (communication status) select
      // because these values will be set via your calendar integration.
      ...
      popupForm.onsubmit = function(e) {
        ...
        const updatedTags = document.getElementById("edit-guest-tags").value.split(",").map(t => t.trim()).filter(Boolean);
        const updatedAreasText = document.getElementById("edit-guest-areas").value;
        const updatedAreas = updatedAreasText ? updatedAreasText.split(",").map(s => s.trim()).filter(Boolean) : [];
  ...
  // After retrieving 'data' for the current guest:
if (!data.inCalendar) {
